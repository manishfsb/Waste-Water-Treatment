"""
ann_modeling_exp2s2.py - ANN (MLPRegressor) on Experiment 2 Sub-2 datasets.

Diagnostic follow-up to Phase 9 ANN (Exp3-S2, avg Test R²=−1.12).
Exp2-Sub2 (Inlet + Secondary + COMMON, 19 features) has ~920 Grab /
~733 Composite training rows vs ~470/290 in Exp3-S2.

Tests the ANN with the full combined feature set (inlet + secondary)
at twice the sample count of the Exp3-S2 run. Directly comparable
to Phase 9 ANN: identical architecture, more data, fewer features.

Outputs:
  - phase9/ann_exp2s2/results.xlsx
  - phase9/ann_exp2s2/models/<name>_ann_run_N.pkl
  - phase9/ann_exp2s2/plots/<name>_ann_run_N_{scatter,timeseries,lc}.png

Usage (from project root):
    .venv/bin/python3 modeling/models/phase9/ann_exp2s2/ann_modeling_exp2s2.py
"""
...
DS_DIR       = os.path.join(MODELING_DIR, "datasets", "experiment2", "sub_exp2")
...
DATASETS = [
    {
        "name":       "stage2_p2_grab_BOD",
        "file":       os.path.join(DS_DIR, "stage2_p2_grab_BOD.xlsx"),
        "target":     "Effluent BOD (mg/L, Grab)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_grab_COD",
        "file":       os.path.join(DS_DIR, "stage2_p2_grab_COD.xlsx"),
        "target":     "Effluent COD (mg/L, Grab)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_grab_TSS",
        "file":       os.path.join(DS_DIR, "stage2_p2_grab_TSS.xlsx"),
        "target":     "Effluent TSS (mg/L, Grab)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_grab_pH",
        "file":       os.path.join(DS_DIR, "stage2_p2_grab_pH.xlsx"),
        "target":     "Effluent pH (Grab)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_comp_BOD",
        "file":       os.path.join(DS_DIR, "stage2_p2_comp_BOD.xlsx"),
        "target":     "Effluent BOD (mg/L, Composite)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_comp_COD",
        "file":       os.path.join(DS_DIR, "stage2_p2_comp_COD.xlsx"),
        "target":     "Effluent COD (mg/L, Composite)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_comp_TSS",
        "file":       os.path.join(DS_DIR, "stage2_p2_comp_TSS.xlsx"),
        "target":     "Effluent TSS (mg/L, Composite)",
        "experiment": "ANN-Exp2-Sub2",
    },
    {
        "name":       "stage2_p2_comp_pH",
        "file":       os.path.join(DS_DIR, "stage2_p2_comp_pH.xlsx"),
...
def _scatter_plot(y_train, y_train_pred, y_test, y_test_pred,
    ...
    fig.suptitle(f"ANN-Exp2-Sub2 - {name}", fontsize=10)
    plt.tight_layout()
...
def _timeseries_plot(df_full, target, y_pred_full, name, run):
    ...
    ax.set_title(f"ANN-Exp2-Sub2 - {target}", fontsize=9)
...
def _learning_curve_plot(pipeline, X_train, y_train, name, run):
    ...
    ax.set_title(f"Learning Curve - ANN-Exp2-Sub2 - {name}", fontsize=9)
...
def main():
    ...
    print(f"=== ANN - Experiment 2 Sub-2 Datasets - Run {run} ===")
