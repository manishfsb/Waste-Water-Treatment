"""
ann_modeling_exp2s1.py - ANN (MLPRegressor) on Experiment 2 Sub-1 datasets.

Diagnostic follow-up to Phase 9 ANN (Exp3-S2, avg Test R²=−1.12).
Exp2-Sub1 (Secondary Clarifier + COMMON, 15 features) has ~924 Grab /
...
Outputs:
  - phase9/ann_exp2s1/results.xlsx
  - phase9/ann_exp2s1/models/<name>_ann_run_N.pkl
  - phase9/ann_exp2s1/plots/<name>_ann_run_N_{scatter,timeseries,lc}.png

Usage (from project root):
    .venv/bin/python3 modeling/models/phase9/ann_exp2s1/ann_modeling_exp2s1.py
"""
...
DS_DIR       = os.path.join(MODELING_DIR, "datasets", "experiment2", "sub_exp1")
...
def _scatter_plot(y_train, y_train_pred, y_test, y_test_pred,
    ...
    fig.suptitle(f"ANN-Exp2-Sub1 - {name}", fontsize=10)
    plt.tight_layout()
...
def _timeseries_plot(df_full, target, y_pred_full, name, run):
    ...
    ax.set_title(f"ANN-Exp2-Sub1 - {target}", fontsize=9)
...
def _learning_curve_plot(pipeline, X_train, y_train, name, run):
    ...
    ax.set_title(f"Learning Curve - ANN-Exp2-Sub1 - {name}", fontsize=9)
...
def main():
    ...
    print(f"=== ANN - Experiment 2 Sub-1 Datasets - Run {run} ===")
