...
Outputs:
  - phase9/ann/results.xlsx
  - phase9/ann/models/<name>_ann_run_N.pkl
  - phase9/ann/plots/<name>_ann_run_N_{scatter,timeseries,lc}.png
  - Appends predicted_ANN_run_N column to each dataset xlsx

Usage (from project root):
    .venv/bin/python3 21-25/modeling/models/phase9/ann/ann_modeling_phase9.py
"""
...
DATASETS = [
    {
        ...
        "experiment": "Phase9-ANN",
        "model_name": "p9_ann_grab_BOD",
    },
    {
        "name":       "grab_COD",
...
def train_dataset(ds: dict, run: int) -> dict:
    ...
    pred_col = f"predicted_ANN_run_{run}"
    df_orig  = pd.read_excel(ds["file"])
    df_orig["Date"] = pd.to_datetime(df_orig["Date"])
    feat_avail = [c for c in feat_cols if c in df_orig.columns]
    target_avail = target in df_orig.columns

    if target_avail and all(c in df_orig.columns for c in feat_avail):
        mask = df_orig[feat_avail + [target]].notna().all(axis=1)
        X_all = df_orig.loc[mask, feat_avail].values
        df_orig.loc[:, pred_col] = np.nan
        df_orig.loc[mask, pred_col] = best.predict(X_all)
