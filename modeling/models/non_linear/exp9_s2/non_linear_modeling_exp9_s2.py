"""
non_linear_modeling_exp9_s2.py - RF, GB, XGBoost on Experiment 9 SE2 datasets (OOF FS).

Recency hypothesis - W1+FS (2024-only training window with OOF permutation importance FS).
3-phase pipeline per model:
  Phase 1: CV on full 27-feature set -> best estimator
  Phase 2: OOF permutation importance (threshold=0.05) -> selected feature mask
  Phase 3: refit on selected features -> final metrics

Feature set: identical to Exp3-SE1 (27 features: Inlet + Secondary + COMMON_CYCLIC
+ ADD-tier aeration). Only the training window and FS step change vs SE1.

Datasets: experiment9/sub_exp1/ (same as SE1)
Exp key : Exp9-SE2

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp9_s2/non_linear_modeling_exp9_s2.py
"""
...
# -- Registry -------------------------------------------------------------------
REGISTRY = [
    ("Exp9-SE2", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp9-SE2", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp9-SE2", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp9-SE2", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp9-SE2", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp9-SE2", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp9-SE2", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp9-SE2", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
# -- OOF permutation importance selection ---------------------------------------
...
def _plot_scatter(plots_dir, name, model_tag, run, test_df, y_test, y_pred, target):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+FS)\nR²={_r2(y_test, y_pred):+.3f}  "
...
def _plot_importance(plots_dir, name, model_tag, run, model, features):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+FS) | Feature Importance (run {run})",
                 fontsize=11)
    plt.tight_layout()
...
def train_one(experiment, name, subset_path, features, target,
    ...
    # Phase 1 - CV on full feature set
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp9-SE2, W1+FS: 2024-only + OOF FS) - run {run}")
    print(f"{'='*65}")
...
# -- Main -----------------------------------------------------------------------
def main():
    print("Non-Linear Modeling  -  Exp9-SE2 (RF / GB / XGB)")
    print("W1+FS: 2024-only training window + OOF permutation importance FS (3-phase)\n")
    ...
    print("\n" + "="*65)
    print("  Exp9-SE2 (W1+FS)  -  all models complete.")
