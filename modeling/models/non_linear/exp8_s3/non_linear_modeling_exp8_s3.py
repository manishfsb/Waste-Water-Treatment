"""
non_linear_modeling_exp8_s3.py - RF, GB, XGBoost on Experiment 10 SE1 datasets.

Feature set: Same-day available features only (22 features).
Exp3-SE1 baseline with BOD and COD input columns removed.

Exp key: Exp8-SE3

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp8_s3/non_linear_modeling_exp8_s3.py
"""
...
# -- Registry ------------------------------------------------------------------
REGISTRY = [
    ("Exp8-SE3", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp8-SE3", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp8-SE3", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp8-SE3", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp8-SE3", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp8-SE3", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp8-SE3", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp8-SE3", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp8-SE3, same-day features) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp8-SE3 (same-day features) - all models complete.")
