"""
non_linear_modeling_exp2_s4.py - RF, GB, XGBoost on Experiment 2 SE3.

Feature set: Core Inlet (4) + Primary (6) + Secondary (10) + COMMON (7) = 27 features.
Tests whether combining all three monitoring points improves over the Exp2 SE3 reference
baseline (Inlet + Secondary, 21 features, ~920 train rows).
...
Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp2_s4/non_linear_modeling_exp2_s4.py
"""
...
REGISTRY = [
    ("Exp2-S4", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S4", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S4", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S4", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S4", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S4", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S4", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S4", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def train_one(experiment, name, subset_path, features, target,
    ...
    col    = f"predicted_{model_tag}_nofs_run_{run}"
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp2-SE3, Inlet + Primary + Secondary + COMMON, 27 feat) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp2-SE3 (Inlet + Primary + Secondary + COMMON, 27 feat)  -  all models complete.")
