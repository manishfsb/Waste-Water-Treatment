"""
non_linear_modeling_exp3_s1_fs.py - RF, GB, XGBoost on Exp3 Sub-1 Feature-Selected subsets.

Identical training protocol to non_linear_modeling_exp3_s1.py.
Differences:
  - Reads from experiment3/sub_exp1/feature_selected_datasets/ (built by build_exp3_s1_fs_subsets.py)
  - Experiment label: "Experiment 3 Sub-1 FS"
  - Writes all artifacts to models/non_linear/exp3_s1_fs/{rf,gb,xgb}/

Usage (from project root):
    .venv/bin/python3 21-25/modeling/models/non_linear/exp3_s1_fs/non_linear_modeling_exp3_s1_fs.py
"""
...
def _e3s1_fs(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment3", "sub_exp1",
                        "feature_selected_datasets", f"{name}.xlsx")
...
def _plot_timeseries(plots_dir, name, model_tag, run, df_all, features, model, target):
    ...
    if len(ts):
        ax.axvspan(ts.min(), ts.max(), alpha=0.10, color="orange", label=f"Test ({TEST_YEAR})")
    ax.plot(df_plot["Date"], df_plot[target], color="#2171B5", linewidth=0.8, label="Actual", alpha=0.9)
    ax.plot(df_plot["Date"], df_plot["_pred"], color=MODEL_COLOURS[model_tag],
            linewidth=0.8, label=f"{model_tag} Predicted", alpha=0.9)
...
def train_one(experiment, name, subset_path, features, target,
    ...
    plots = {
        "scatter":    _plot_scatter(plots_dir, name, model_tag, run, test, y_te, te_pred, target),
        "timeseries": _plot_timeseries(plots_dir, name, model_tag, run, df, features, best, target),
        "importance": _plot_importance(plots_dir, name, model_tag, run, best, features),
    }
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} - Exp3 Sub-1 (Feature Selected) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  All models complete (Exp3 Sub-1 Feature Selected).")
