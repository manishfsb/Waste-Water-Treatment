"""
non_linear_modeling_exp2_s1.py

Trains RF, GradientBoosting, and XGBoost on Experiment 2 Sub-experiment 1 combined datasets.
...
Baseline runs (no OOF feature selection). Single model trained per dataset.

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp2_s1/non_linear_modeling_exp2_s1.py
"""
...
def train_one(experiment, name, subset_path, features, target,
    ...
    # Single CV search on full feature set (no OOF FS for baseline runs)
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    print(f"\n{'='*60}")
    print(f"{model_tag}  -  Exp2-Sub1  (run {run})")
    print(f"{'='*60}")
...
def main():
    print("Non-Linear Modeling  -  Exp2-Sub1  (RF / GB / XGB)")
    print("17 features · SEC_COLS + COMMON_CYCLIC · No OOF FS (baseline runs)\n")
