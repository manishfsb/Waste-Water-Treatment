"""
non_linear_modeling_exp5_s2_fs.py - RF, GB, XGB on Experiment 5 Sub-2 (OOF FS).
...
Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp5_s2_fs/non_linear_modeling_exp5_s2_fs.py
"""
...
REGISTRY = [
    ("Exp5-S2-FS", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp5-S2-FS", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp5-S2-FS", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp5-S2-FS", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
]
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    print(f"\n{'='*60}")
    print(f"{model_tag}  -  Exp5-S2-FS (OOF FS on Grab + Comp Inlet, run {run})")
    print(f"{'='*60}")
...
def main():
    print("Non-Linear Modeling  -  Exp5-S2-FS (RF / GB / XGB)")
    print("Grab + Comp Inlet features  ·  OOF permutation importance FS (3-phase)\n")
