"""
non_linear_modeling_exp2_s3.py - RF, GB, XGBoost on Experiment 2 SE3.

Feature set: All primary stage features (6) + GRAB/COMP_INLET (4) + COMMON_CYCLIC (7) = 17 features.
Train sizes: ~778 grab / ~608 comp. Trees are unaffected by VIF warnings on Primary pH/BOD/COD.

Datasets: experiment2/sub_exp3/ (built by make_sub3_datasets.py)
Exp key:  Exp2-S3

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp2_s3/non_linear_modeling_exp2_s3.py
"""
...
REGISTRY = [
    ("Exp2-S3", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S3", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S3", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S3", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S3", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S3", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S3", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S3", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp2-S3, Primary + Inlet + COMMON_CYCLIC) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp2-SE3 (Primary + Inlet + COMMON_CYCLIC)  -  all models complete.")
