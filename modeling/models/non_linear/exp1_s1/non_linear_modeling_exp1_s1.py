"""
non_linear_modeling_exp1_s1.py

Trains RF, GradientBoosting, and XGBoost on Experiment 1 Sub-1 datasets:
inlet features only, no Flow / Power / calendar columns.
...
Feature set (4 features per target):
  Grab: Inlet pH, BOD, COD, TSS (Grab)
  Comp: Inlet pH, BOD, COD, TSS (Composite)

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp1_s1/non_linear_modeling_exp1_s1.py
"""
...
S1_GRAB = GRAB_INLET   # 4 features
S1_COMP = COMP_INLET   # 4 features

# ── Dataset registry ───────────────────────────────────────────────────────────
def _s1(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment1", "sub_exp1", f"{name}.xlsx")


REGISTRY = [
    ("Exp1-Sub1", "grab_BOD", _s1("grab_BOD"), S1_GRAB, "Effluent BOD (mg/L, Grab)"),
    ("Exp1-Sub1", "grab_COD", _s1("grab_COD"), S1_GRAB, "Effluent COD (mg/L, Grab)"),
    ("Exp1-Sub1", "grab_TSS", _s1("grab_TSS"), S1_GRAB, "Effluent TSS (mg/L, Grab)"),
    ("Exp1-Sub1", "grab_pH",  _s1("grab_pH"),  S1_GRAB, "Effluent pH (Grab)"),
    ("Exp1-Sub1", "comp_BOD", _s1("comp_BOD"), S1_COMP, "Effluent BOD (mg/L, Composite)"),
    ("Exp1-Sub1", "comp_COD", _s1("comp_COD"), S1_COMP, "Effluent COD (mg/L, Composite)"),
    ("Exp1-Sub1", "comp_TSS", _s1("comp_TSS"), S1_COMP, "Effluent TSS (mg/L, Composite)"),
    ("Exp1-Sub1", "comp_pH",  _s1("comp_pH"),  S1_COMP, "Effluent pH (Composite)"),
]
...
def _plot_scatter(plots_dir, name, model_tag, run, test_df, y_test, y_pred):
...
def train_one(experiment, name, subset_path, features, target,
    ...
    print(f"  done  R²_test={row['R2_test']:+.3f}  gap={row['R2_gap']:+.3f}  "
          f"params={gs.best_params_}")

    joblib.dump(best, os.path.join(model_dir, f"{name}_{model_tag}_run_{run}.pkl"))
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    for experiment, name, path, features, target in REGISTRY:
        ...
        if not os.path.exists(path):
            ...
            print(f"    Run make_sub1_datasets.py first.")
            continue
