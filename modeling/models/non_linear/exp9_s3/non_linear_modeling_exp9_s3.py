"""
non_linear_modeling_exp9_s3.py - RF, GB, XGBoost on Experiment 9 SE3 datasets.

Recency hypothesis - W1 + log1p target transformation.
Feature set: identical to Exp9-SE1 (27 features). BOD/COD/TSS targets are
log1p-transformed before fitting; pH is left on its natural scale.
Duan's smearing estimator back-transforms predictions to the original scale.
...
Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp9_s3/non_linear_modeling_exp9_s3.py
"""
...
REGISTRY = [
    ("Exp9-SE3", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)",       True),
    ("Exp9-SE3", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)",       True),
    ("Exp9-SE3", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)",       True),
    ("Exp9-SE3", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)",              False),
    ("Exp9-SE3", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)",  True),
    ("Exp9-SE3", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)",  True),
    ("Exp9-SE3", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)",  True),
    ("Exp9-SE3", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)",         False),
]
...
def _plot_scatter(plots_dir, name, model_tag, run, test_df, y_test, y_pred, target):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+LogY)\nR²={_r2(y_test, y_pred):+.3f}  "
...
def _plot_importance(plots_dir, name, model_tag, run, model, features):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+LogY) | Feature Importance (run {run})",
                 fontsize=11)
    plt.tight_layout()
...
def train_one(experiment, name, subset_path, features, target, log_y,
    ...
    print(f"    n_train={len(train)} n_test={len(test)} n_feat={len(features)} "
          f"log_y={log_y} - tuning...", end="", flush=True)
    ...
    if log_y:
        ...
        # For full-dataset predictions stored in xlsx
        df_sub  = pd.read_excel(subset_path)
        all_pred_t = best.predict(df_sub[features].values)
        all_pred = _back_transform(all_pred_t, smear)
    else:
        smear = 1.0
        tr_pred = tr_pred_t; te_pred = te_pred_t
        df_sub  = pd.read_excel(subset_path)
        all_pred = best.predict(df_sub[features].values)
    ...
    col = f"predicted_{model_tag}_log_run_{run}"
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp9-SE3, W1+LogY: 2024-only + log1p targets) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp9-SE3 (W1+LogY)  -  all models complete.")
