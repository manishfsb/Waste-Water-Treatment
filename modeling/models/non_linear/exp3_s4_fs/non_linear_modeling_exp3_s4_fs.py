"""
non_linear_modeling_exp3_s4_fs.py - RF, GB, XGB on Experiment 3 KS-FS (OOF FS).

Reads the all-features datasets from experiment3/sub_exp4/ for Phase 1+2 only:
...
Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp3_s4_fs/non_linear_modeling_exp3_s4_fs.py
"""
...
# ── Dataset registry ───────────────────────────────────────────────────────────
REGISTRY = [
    ("Exp3-S4-FS", "grab_BOD", _e3s1("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp3-S4-FS", "grab_COD", _e3s1("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp3-S4-FS", "grab_TSS", _e3s1("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp3-S4-FS", "grab_pH",  _e3s1("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp3-S4-FS", "comp_BOD", _e3s1("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp3-S4-FS", "comp_COD", _e3s1("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp3-S4-FS", "comp_TSS", _e3s1("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp3-S4-FS", "comp_pH",  _e3s1("comp_pH"),  "Effluent pH (Composite)"),
]
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    print(f"\n{'='*60}")
    print(f"{model_tag}  -  Exp3-S4-FS (OOF FS on all-features, run {run})")
    print(f"{'='*60}")
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    print("Non-Linear Modeling  -  Exp3-S4-FS (RF / GB / XGB)")
