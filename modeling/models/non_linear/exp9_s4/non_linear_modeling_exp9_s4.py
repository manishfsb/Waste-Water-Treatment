"""
non_linear_modeling_exp9_s4.py - RF, GB, XGBoost on Experiment 9 SE4 datasets.
...
Target remains on original scale - no back-transformation needed.

Datasets: experiment9/sub_exp1/ (same as SE1)
Exp key : Exp9-SE4

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp9_s4/non_linear_modeling_exp9_s4.py
"""
...
# -- Same transform set as Exp7 (Phase 10), minus coliform/primary sludge ------
LOG_FEATURES = {
    "Inlet BOD (mg/L, Grab)",
    "Inlet COD (mg/L, Grab)",
    "Inlet TSS (mg/L, Grab)",
...
# -- Registry -------------------------------------------------------------------
REGISTRY = [
    ("Exp9-SE4", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp9-SE4", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp9-SE4", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp9-SE4", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp9-SE4", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp9-SE4", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp9-SE4", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp9-SE4", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_scatter(plots_dir, name, model_tag, run, test_df, y_test, y_pred, target):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+LogF)\nR²={_r2(y_test, y_pred):+.3f}  "
...
def _plot_importance(plots_dir, name, model_tag, run, model, features):
    ...
    ax.set_title(f"{name} - {model_tag} (W1+LogF) | Feature Importance (run {run})",
                 fontsize=11)
    plt.tight_layout()
...
def train_one(experiment, name, subset_path, features, target,
    ...
    print(f"    n_train={len(train)} n_test={len(test)} n_feat={len(features)} "
          f"log_transformed={len(transformed)} - tuning...", end="", flush=True)
    ...
    # Store predictions in the original dataset file (untransformed targets)
    ...
    df_sub[f"predicted_{model_tag}_logf_run_{run}"] = np.round(all_pred, 3)
    df_sub.to_excel(subset_path, index=False)

    joblib.dump({"model": best, "log_features": transformed, "features": features},
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp9-SE4, W1+LogF: 2024-only + log1p features) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp9-SE4 (W1+LogF)  -  all models complete.")
