"""
non_linear_modeling_exp5_s2.py - RF, GB, XGBoost on Experiment 5 Sub-2.

Hypothesis: adding Composite inlet measurements to grab-target datasets
improves grab effluent prediction quality.

Feature set: GRAB_INLET + COMP_INLET + Secondary + COMMON + ADD-tier aeration
             = 32 features per grab dataset
Datasets:    experiment5/sub_exp2/ (4 grab targets)
Exp key:     Exp5-S2

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp5_s2/non_linear_modeling_exp5_s2.py
"""
...
REGISTRY = [
    ("Exp5-S2", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp5-S2", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp5-S2", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp5-S2", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
]
...
def train_one(experiment, name, subset_path, features, target,
    ...
    print(f"  CV_RMSE={cv_rmse:.3f}  best={search.best_params_}")

    tr_pred = best.predict(X_tr); te_pred = best.predict(X_te)

    row = {
        "experiment": experiment, "model_name": name, "run": run,
        "model": model_tag, "target": target,
    ...
    joblib.dump(best, os.path.join(models_dir, f"{name}_{model_tag}_run_{run}.pkl"))
    ...
    df_sub[f"predicted_{model_tag}_NL_run_{run}"] = best.predict(df_sub[features].values).round(3)
...
def run_model(model_tag, search_factory):
    ...
    print(f"\n{'='*65}")
    print(f"  {model_tag} (Exp5-S2, Grab + Composite Inlet) - run {run}")
    print(f"{'='*65}")
...
def main():
    ...
    print("\n" + "="*65)
    print("  Exp5-S2  -  all models complete.")
