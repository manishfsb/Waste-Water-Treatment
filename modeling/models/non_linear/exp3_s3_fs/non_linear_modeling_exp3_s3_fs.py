"""
non_linear_modeling_exp3_s3_fs.py - RF, GB, XGB on Experiment 3 Sub-3 (OOF FS).

Reads the SE3 datasets (ADD+CONSIDER minus Inlet Total Coliform, 30 features) from
experiment3/sub_exp3/ and applies 3-phase OOF permutation importance feature selection:
  Phase 1 : CV search on all features -> best estimator + full R²/RMSE
  ...
  R2_test_full / R2_gap_full store Phase 1 (pre-selection) results.

Exp key: Exp3-S3-FS

Usage (from project root):
    .venv/bin/python3 modeling/models/non_linear/exp3_s3_fs/non_linear_modeling_exp3_s3_fs.py
"""
...
REGISTRY = [
    ("Exp3-S3-FS", "grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp3-S3-FS", "grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp3-S3-FS", "grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp3-S3-FS", "grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp3-S3-FS", "comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp3-S3-FS", "comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp3-S3-FS", "comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp3-S3-FS", "comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def run_model(model_tag: str, estimator, param_grid, search_cls):
    ...
    print(f"\n{'='*60}")
    print(f"{model_tag}  -  Exp3-S3-FS (OOF FS on ADD+CONSIDER minus Coliform, run {run})")
    print(f"{'='*60}")
...
def main():
    print("Non-Linear Modeling  -  Exp3-S3-FS (RF / GB / XGB)")
    print("ADD+CONSIDER minus Coliform  ·  OOF permutation importance FS (3-phase)\n")
