"""
linear_modeling_exp5_s2_fs.py - OLS (LassoCV FS), Ridge, ElNet on Experiment 5 Sub-2.

Hypothesis: adding Grab inlet measurements to composite-target datasets improves
composite effluent prediction quality. Feature selection identifies the most
informative columns from the expanded 32-feature set.

Feature set: COMP_INLET + GRAB_INLET + Secondary + COMMON + ADD-tier aeration = 32 features.
Model-specific FS:
  - OLS  : LassoCV pre-screen (TimeSeriesSplit); stores pre-FS metrics as OLS_full_*
...
Datasets: experiment5/sub_exp2/ (4 grab targets)
Exp key:  Exp5-S2-FS

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp5_s2_fs/linear_modeling_exp5_s2_fs.py
"""
...
def _ds(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment5", "sub_exp2", f"{name}.xlsx")

DATASETS = [
    ("Exp5-S2-FS", "Exp5S2_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp5-S2-FS", "Exp5S2_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp5-S2-FS", "Exp5S2_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp5-S2-FS", "Exp5S2_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
]
...
def train_dataset(experiment, ds_id, path, features, target, run):
    ...
    print(f"  Train: {len(train_df):>4d} rows | Test: {len(test_df):>3d} rows | Features: {n_in}")
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp5-S2-FS (Grab + Comp Inlet, LassoCV FS)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp5-S2-FS_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp5-S2-FS (Grab + Comp Inlet, LassoCV FS)")
    ...
    print(f"\nExp5-S2-FS Linear Modeling  -  Run {run} complete")
