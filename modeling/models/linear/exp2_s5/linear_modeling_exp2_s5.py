"""
linear_modeling_exp2_s5.py - OLS, Ridge, ElNet on Experiment 2 SE1a: Primary + COMMON only.

Feature set: PRIMARY (6) + COMMON (7) = 13 features.
Isolates primary-stage signal without inlet features. Contrasts with exp2_s3
(Primary + Inlet + COMMON, 17 features) to determine whether inlet adds or
destroys signal in the primary stage.

VIF warnings for linear models:
  Primary Clarifier pH: VIF ~2743 (near-perfect collinearity with other pH cols)
  Primary BOD:          VIF ~17.8
  Primary COD:          VIF ~21.6
  ElNet (L1) is more robust to these than OLS/Ridge.

Datasets: experiment2/sub_exp5/ (built by make_sub5_datasets.py)
Exp key:  Exp2-S5

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s5/linear_modeling_exp2_s5.py
"""
...
def _ds(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment2", "sub_exp5", f"{name}.xlsx")

DATASETS = [
    ("Exp2-S5", "E2S5_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S5", "E2S5_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S5", "E2S5_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S5", "E2S5_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S5", "E2S5_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S5", "E2S5_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S5", "E2S5_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S5", "E2S5_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def get_run_number(subset_path: str) -> int:
    ...
    return sum(1 for c in cols if c.startswith("predicted_OLS_nofs_run_")) + 1
...
def _plot_scatter(ds_id, run, y_test, te_ols, te_ridge, te_elnet):
    ...
    pairs = [("OLS", te_ols, MODEL_COLORS["OLS"]),
    ...
    for ax, (label, preds_arr, color) in zip(axes, pairs):
        ...
        ax.set_title(f"{label}\nR2={r2_score(y_test, preds_arr):+.3f}  "
                     f"RMSE={_rmse(y_test, preds_arr):.3f}", fontsize=11)
...
def train_dataset(experiment, ds_id, path, features, target, run):
    ...
    preds[f"predicted_OLS_nofs_run_{run}"] = np.round(ols.predict(X_all_sc), 3)
    joblib.dump({"scaler": scaler, "model": ols},
                os.path.join(MODELS_DIR, f"{ds_id}_OLS_run_{run}.pkl"))
    print(f"    OLS    - Train R2: {results['OLS_train_R2']:+.3f} | "
          f"Test R2: {results['OLS_test_R2']:+.3f} | RMSE: {results['OLS_test_RMSE']:.3f}")
    ...
    preds[f"predicted_Ridge_nofs_run_{run}"] = np.round(ridge.predict(X_all_sc), 3)
    joblib.dump({"scaler": scaler, "model": ridge},
                os.path.join(MODELS_DIR, f"{ds_id}_Ridge_run_{run}.pkl"))
    print(f"    Ridge  - Train R2: {results['Ridge_train_R2']:+.3f} | "
          f"Test R2: {results['Ridge_test_R2']:+.3f} | alpha={results['Ridge_alpha']}")
    ...
    preds[f"predicted_ElNet_nofs_run_{run}"] = np.round(elnet.predict(X_all_sc), 3)
    joblib.dump({"scaler": scaler, "model": elnet},
                os.path.join(MODELS_DIR, f"{ds_id}_ElNet_run_{run}.pkl"))
    print(f"    ElNet  - Train R2: {results['ElNet_train_R2']:+.3f} | "
          f"Test R2: {results['ElNet_test_R2']:+.3f} | "
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp2-SE1a (Primary + COMMON, 13 features)  |  Test R2 (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-S5_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp2-SE1a (Primary + COMMON, 13 features)  -  Run {run}")
