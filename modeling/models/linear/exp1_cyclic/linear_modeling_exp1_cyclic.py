"""
linear_modeling_exp1_cyclic.py

Trains OLS, Ridge, and ElasticNet on Experiment 1 datasets where the raw
integer month and day_of_week columns have been replaced with cyclic sin/cos
encodings (month_sin, month_cos, dow_sin, dow_cos).

Compares against the baseline Exp1 results to measure whether proper cyclic
encoding of calendar features improves generalisation.

Feature set (11 features per target):
  Grab: Inlet pH, BOD, COD, TSS (Grab) + Flow, Power, year, month_sin, month_cos, dow_sin, dow_cos
  Comp: Inlet pH, BOD, COD, TSS (Composite) + Flow, Power, year, month_sin, month_cos, dow_sin, dow_cos

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp1_cyclic/linear_modeling_exp1_cyclic.py
"""
...
# month and day_of_week replaced with 4 cyclic columns
COMMON_CYCLIC = [
    "Flow (MLD)", "Power Total (KW)", "year",
    "month_sin", "month_cos", "dow_sin", "dow_cos",
]

EXP1_GRAB = GRAB_INLET + COMMON_CYCLIC   # 11 features
EXP1_COMP = COMP_INLET + COMMON_CYCLIC   # 11 features
...
DATASETS = [
    ("Exp1-Cyclic", "E1Cyc_Grab_BOD", _cyc("grab_BOD"), EXP1_GRAB, "Effluent BOD (mg/L, Grab)"),
    ("Exp1-Cyclic", "E1Cyc_Grab_COD", _cyc("grab_COD"), EXP1_GRAB, "Effluent COD (mg/L, Grab)"),
    ("Exp1-Cyclic", "E1Cyc_Grab_TSS", _cyc("grab_TSS"), EXP1_GRAB, "Effluent TSS (mg/L, Grab)"),
    ("Exp1-Cyclic", "E1Cyc_Grab_pH",  _cyc("grab_pH"),  EXP1_GRAB, "Effluent pH (Grab)"),
    ("Exp1-Cyclic", "E1Cyc_Comp_BOD", _cyc("comp_BOD"), EXP1_COMP, "Effluent BOD (mg/L, Composite)"),
    ("Exp1-Cyclic", "E1Cyc_Comp_COD", _cyc("comp_COD"), EXP1_COMP, "Effluent COD (mg/L, Composite)"),
    ("Exp1-Cyclic", "E1Cyc_Comp_TSS", _cyc("comp_TSS"), EXP1_COMP, "Effluent TSS (mg/L, Composite)"),
    ("Exp1-Cyclic", "E1Cyc_Comp_pH",  _cyc("comp_pH"),  EXP1_COMP, "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp1-Cyclic  |  Test R² Comparison (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
        df_results[["OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp1-Cyclic_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"Exp1-Cyclic Linear Modeling - Run {run}")
    print(f"Features: {len(EXP1_GRAB)} per target  "
          f"(raw month/dow replaced with month_sin/cos, dow_sin/cos)\n")
