"""
linear_modeling_exp2_s6.py - OLS, Ridge, ElNet on Experiment 2 SE2a: All Secondary only.

Feature set: SECONDARY (10) = Sec Clarifier (5) + Sec Sedimentation (5). No COMMON, no Inlet.
Isolates secondary-stage signal without plant-level operational context (Flow, Power, calendar).
Contrasts with exp2_s1 (All Secondary + COMMON, Exp2-Sub1) to attribute signal sources.

Datasets: experiment2/sub_exp6/ (built by make_sub6_datasets.py)
Exp key:  Exp2-S6

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s6/linear_modeling_exp2_s6.py
"""
...
DATASETS = [
    ("Exp2-S6", "E2S6_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S6", "E2S6_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S6", "E2S6_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S6", "E2S6_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S6", "E2S6_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S6", "E2S6_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S6", "E2S6_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S6", "E2S6_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp2-SE2a (All Secondary only, 10 features)  |  Test R2 (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-S6_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp2-SE2a (All Secondary only, 10 features)  -  Run {run}")
