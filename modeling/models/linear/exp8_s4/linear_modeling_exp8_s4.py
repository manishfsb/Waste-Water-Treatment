"""
linear_modeling_exp8_s4.py - OLS, Ridge, ElasticNet on Experiment 10 SE1 datasets.

Feature set: Same-day available features only (22 features).
Exp3-SE1 baseline with BOD and COD input columns removed - these require
5-day lab incubation and are not available on the day of sampling.

Remaining features: Inlet pH/TSS + Sec pH/TSS/RAS (x2) + ADD-tier Aeration
(DO/MLSS/SV30/SVI/pH Existing + DO/SV30 New) + Flow + Power + COMMON_CYCLIC.

Exp key: Exp8-SE4

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp8_s4/linear_modeling_exp8_s4.py
"""
...
# -- Dataset registry ----------------------------------------------------------
def _ds(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment8", "sub_exp4", f"{name}.xlsx")

DATASETS = [
    ("Exp8-SE4", "Exp8SE4_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp8-SE4", "Exp8SE4_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp8-SE4", "Exp8SE4_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp8-SE4", "Exp8SE4_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp8-SE4", "Exp8SE4_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp8-SE4", "Exp8SE4_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp8-SE4", "Exp8SE4_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp8-SE4", "Exp8SE4_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def main():
    ...
    print(f"Linear Modeling  -  Exp8-SE4 (same-day features + lag-5 BOD/COD)  -  Run {run}")
