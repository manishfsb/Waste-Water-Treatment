"""
linear_modeling_exp1_s3_fs.py - OLS (LassoCV FS), Ridge, ElNet on Experiment 1 SE3.
...
and applies model-specific feature selection:
  - OLS  : LassoCV pre-screen; stores pre-FS full metrics as R2_test_full / OLS_full_*
...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp1_s3_fs/linear_modeling_exp1_s3_fs.py
"""
...
def _lasso_select(X_tr_sc, y_train, features, tscv):
    ...
    print(f"  -  dropped: {dropped}" if dropped else " (no pruning)")
...
def append_predictions(subset_path, pred_dict):
...
def _plot_r2_barchart(df_results, run):
    ...
    ax.set_title(f"Exp1-SE3-FS (LassoCV FS on all grab inlet + COMMON_CYCLIC)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp1-S3-FS_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def save_results(all_results, run):
    ...
    print(df_new[["experiment","dataset","OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].to_string(index=False))

def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp1-SE3-FS (LassoCV FS on all grab inlet + COMMON_CYCLIC)  -  Run {run}")
    print(f"Datasets: {len(DATASETS)}  |  Models: OLS (LassoCV), Ridge, ElasticNet\n")

    all_results = []
    for experiment, ds_id, path, target in DATASETS:
        print(f"{'─'*60}")
        print(f"[{experiment}]  {ds_id}  |  {target}")
    ...
    _plot_r2_barchart(pd.DataFrame(all_results), run)
