"""
linear_modeling_exp5_s2.py - OLS, Ridge, ElasticNet on Experiment 5 Sub-2.
...
Row info (from row_cost_analysis.py):
  Grab baseline (Exp3-S1): ~1018-1021 train rows
  After adding COMP_INLET: ~630-631 train rows (marginal cost ~38.2%)
  Note: high cost is unavoidable - reflects joint grab + composite sampling days.

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp5_s2/linear_modeling_exp5_s2.py
"""
...
DATASETS = [
    ("Exp5-S2", "Exp5S2_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp5-S2", "Exp5S2_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp5-S2", "Exp5S2_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp5-S2", "Exp5S2_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
]
...
def main():
    ...
    print(f"Linear Modeling  -  Exp5-S2 (Grab + Composite Inlet)  -  Run {run}")
