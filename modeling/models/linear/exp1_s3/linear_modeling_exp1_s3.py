...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp1_s3/linear_modeling_exp1_s3.py
"""
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp1-SE3 (All Grab Inlet + COMMON_CYCLIC, 16 features)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp1-S3_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp1-SE3 (All Grab Inlet + COMMON_CYCLIC)  -  Run {run}")
