"""
linear_modeling_exp4_s2.py - OLS, Ridge, and ElasticNet on Experiment 4 Sub-2 subsets.

Experiment 4 Phase 2 hypothesis: automated iterative VIF pruning (threshold=10)
within the Exp4-S1 feature pool further reduces multicollinearity, improving
linear model generalisation and enabling regularisation to work on a well-conditioned
feature matrix.

Dropped vs Exp4-S1 (per target, by VIF pruning):
  - All pH features (Inlet pH, Sec Clarifier pH, Aeration pH, Primary Clarifier pH)
    removed - VIF 600-2400, perfectly correlated across plant
  - Inlet BOD, Inlet COD removed from most targets - VIF 18-34
  - MLSS removed - VIF 30-36 (SV30 survives as proxy for biomass)
  - Flow or Power Total removed (correlated pair)

Gain: +47 to +293 additional rows vs Exp4-S1 (dropped high-missingness features
unlocked more complete observation days).

All other protocol details identical to linear_modeling_exp4_s1.py.

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp4_s2/linear_modeling_exp4_s2.py
"""
...
def _cyclic_features(df: pd.DataFrame) -> tuple:
    ...
    return X, ["month_sin", "month_cos", "dow_sin", "dow_cos"]
...
DATASETS = [
    ("Exp4-S2", "Exp4S2_Grab_BOD", _e4s2("e4_s2_grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp4-S2", "Exp4S2_Grab_COD", _e4s2("e4_s2_grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp4-S2", "Exp4S2_Grab_TSS", _e4s2("e4_s2_grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp4-S2", "Exp4S2_Grab_pH",  _e4s2("e4_s2_grab_pH"),  "Effluent pH (Grab)"),
    ("Exp4-S2", "Exp4S2_Comp_BOD", _e4s2("e4_s2_comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp4-S2", "Exp4S2_Comp_COD", _e4s2("e4_s2_comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp4-S2", "Exp4S2_Comp_TSS", _e4s2("e4_s2_comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp4-S2", "Exp4S2_Comp_pH",  _e4s2("e4_s2_comp_pH"),  "Effluent pH (Composite)"),
]
...
def save_results(all_results: list, run: int):
    ...
    key_cols = ["experiment", "dataset",
                ...
                "OLS_R2_gap",    "Ridge_R2_gap",     "ElNet_R2_gap"]
...
def main():
    ...
    print(f"Linear Modeling Exp4-S2 (VIF-pruned) - Run {run}")
