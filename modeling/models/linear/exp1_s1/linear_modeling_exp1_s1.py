...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp1_s1/linear_modeling_exp1_s1.py
"""
...
EXP1S1_GRAB = GRAB_INLET   # 4 features
EXP1S1_COMP = COMP_INLET   # 4 features
...
DATASETS = [
    ("Exp1-Sub1", "E1S1_Grab_BOD", _s1("grab_BOD"), EXP1S1_GRAB, "Effluent BOD (mg/L, Grab)"),
    ("Exp1-Sub1", "E1S1_Grab_COD", _s1("grab_COD"), EXP1S1_GRAB, "Effluent COD (mg/L, Grab)"),
    ("Exp1-Sub1", "E1S1_Grab_TSS", _s1("grab_TSS"), EXP1S1_GRAB, "Effluent TSS (mg/L, Grab)"),
    ("Exp1-Sub1", "E1S1_Grab_pH",  _s1("grab_pH"),  EXP1S1_GRAB, "Effluent pH (Grab)"),
    ("Exp1-Sub1", "E1S1_Comp_BOD", _s1("comp_BOD"), EXP1S1_COMP, "Effluent BOD (mg/L, Composite)"),
    ("Exp1-Sub1", "E1S1_Comp_COD", _s1("comp_COD"), EXP1S1_COMP, "Effluent COD (mg/L, Composite)"),
    ("Exp1-Sub1", "E1S1_Comp_TSS", _s1("comp_TSS"), EXP1S1_COMP, "Effluent TSS (mg/L, Composite)"),
    ("Exp1-Sub1", "E1S1_Comp_pH",  _s1("comp_pH"),  EXP1S1_COMP, "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp1-Sub1 (Inlet Only)  |  Test R² Comparison (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
        df_results[["OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp1S1_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
