"""
linear_modeling_exp5_s1_fs.py - OLS (LassoCV FS), Ridge, ElNet on Experiment 5 Sub-1.
...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp5_s1_fs/linear_modeling_exp5_s1_fs.py
"""
...
DATASETS = [
    ("Exp5-S1-FS", "Exp5S1_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp5-S1-FS", "Exp5S1_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp5-S1-FS", "Exp5S1_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp5-S1-FS", "Exp5S1_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp5-S1-FS (Comp + Grab Inlet, LassoCV FS)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp5-S1-FS_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp5-S1-FS (Comp + Grab Inlet, LassoCV FS)")
    ...
    print(f"\nExp5-S1-FS Linear Modeling  -  Run {run} complete")
