"""
linear_modeling_exp2_s3.py - OLS, Ridge, ElNet on Experiment 2 SE3.

Feature set: All primary stage features (6) + GRAB/COMP_INLET (4) + COMMON_CYCLIC (7) = 17 features.
Tests whether primary treatment data can predict effluent quality at comparable row cost
to Exp2-SE1 (secondary, ~920 rows). Train sizes: ~778 grab / ~608 comp.
...
VIF warnings for linear models:
  ...
  Coefficients for these features are unreliable in OLS/Ridge. ElNet (L1) is more robust.
  Tree models (non-linear script) are unaffected by VIF.

Datasets: experiment2/sub_exp3/ (built by make_sub3_datasets.py)
Exp key:  Exp2-S3

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s3/linear_modeling_exp2_s3.py
"""
...
def _ds(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment2", "sub_exp3", f"{name}.xlsx")

DATASETS = [
    ("Exp2-S3", "E2S3_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S3", "E2S3_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S3", "E2S3_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S3", "E2S3_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S3", "E2S3_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S3", "E2S3_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S3", "E2S3_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S3", "E2S3_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp2-SE3 (Primary + Inlet + COMMON_CYCLIC, 17 features)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-S3_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp2-SE3 (Primary + Inlet + COMMON_CYCLIC)  -  Run {run}")
