"""
linear_modeling_exp9_s2.py - OLS (LassoCV FS), Ridge, ElNet on Experiment 9 SE2 datasets.

Recency hypothesis - W1+FS (2024-only training window with model-specific feature selection).
Feature set: identical to Exp3-SE1 (27 features), but LassoCV prunes low-signal features
for OLS; Ridge uses full set; ElNet selects internally via L1 penalty.

Hypothesis: removing low-signal features from the small W1 window (187 grab / 179 comp rows)
improves generalisation by reducing noise in the feature space.

Datasets: experiment9/sub_exp1/ (same as SE1)
Exp key : Exp9-SE2

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp9_s2/linear_modeling_exp9_s2.py
"""
...
DATASETS = [
    ("Exp9-SE2", "Exp9W1FS_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp9-SE2", "Exp9W1FS_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp9-SE2", "Exp9W1FS_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp9-SE2", "Exp9W1FS_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp9-SE2", "Exp9W1FS_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp9-SE2", "Exp9W1FS_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp9-SE2", "Exp9W1FS_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp9-SE2", "Exp9W1FS_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _lasso_select(X_tr_sc: np.ndarray, y_train: np.ndarray,
    ...
    print(f"    LassoCV -> {n_kept}/{n_in} features kept", end="")
...
def train_dataset(experiment, ds_id, path, features, target, run):
    ...
    # OLS full (pre-LassoCV baseline for comparison)
    ...
    print(f"    Ridge  - Train R²: {results['Ridge_train_R2']:+.3f} | "
          ...
          f"alpha={results['Ridge_alpha']}  (full {n_in} features)")

    # ElasticNet (full set; L1 selects internally)
    ...
    print(f"    ElNet  - Train R²: {results['ElNet_train_R2']:+.3f} | "
          ...
          f"alpha={results['ElNet_alpha']} | kept {results['ElNet_n_selected']}/{n_in}")
...
def _plot_scatter(ds_id, run, y_test, te_ols, te_ridge, te_elnet):
    ...
    fig.suptitle(f"{ds_id}  |  Test Set  -  W1+FS (run {run})",
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp9-SE2 (W1+FS: 2024-only + LassoCV)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
                df_results[["OLS_test_R2", "Ridge_test_R2", "ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp9SE2_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp9-SE2 (W1+FS: 2024-only + LassoCV feature selection)")
    print(f"Run {run}  |  Datasets: {len(DATASETS)}  |  Models: OLS (LassoCV FS), Ridge, ElNet\n")
    ...
    print(f"\nExp9-SE2 Linear Modeling  -  Run {run} complete.")
