"""
linear_modeling_exp2_s4.py - OLS, Ridge, ElNet on Experiment 2 SE3.

Feature set: Core Inlet (4) + Primary (6) + Secondary (10) + COMMON (7) = 27 features.
Tests whether combining all three monitoring points (inlet, primary, secondary) improves
over the Exp2 SE3 reference baseline (Inlet + Secondary, 21 features).
Train sizes: ~764 grab / ~595 comp.

VIF warnings for linear models (inherited from SE1/Primary features):
  Primary Clarifier pH: VIF ~2743 (near-perfect collinearity with other pH cols)
  Primary BOD:          VIF ~17.8
  Primary COD:          VIF ~21.6
  These are managed by ElasticNet (L1 shrinkage) and Ridge (L2). OLS coefficients
  for these features are unreliable. Tree models are unaffected.

Datasets: experiment2/sub_exp4/ (built by make_sub4_datasets.py)
Exp key:  Exp2-S4

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s4/linear_modeling_exp2_s4.py
"""
...
def _ds(name):
    return os.path.join(MODELING_DIR, "datasets", "experiment2", "sub_exp4", f"{name}.xlsx")

DATASETS = [
    ("Exp2-S4", "E2S4_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp2-S4", "E2S4_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp2-S4", "E2S4_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp2-S4", "E2S4_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp2-S4", "E2S4_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp2-S4", "E2S4_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp2-S4", "E2S4_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp2-S4", "E2S4_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def train_dataset(experiment, ds_id, path, features, target, run):
    ...
    print(f"    OLS    - Train R²: {results['OLS_train_R2']:+.3f} | "
          f"Test R²: {results['OLS_test_R2']:+.3f} | RMSE: {results['OLS_test_RMSE']:.3f}")
    ...
    print(f"    Ridge  - Train R²: {results['Ridge_train_R2']:+.3f} | "
          f"Test R²: {results['Ridge_test_R2']:+.3f} | alpha={results['Ridge_alpha']}")
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp2-SE3 (Inlet + Primary + Secondary + COMMON, 27 features)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-S4_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp2-SE3 (Inlet + Primary + Secondary + COMMON, 27 features)  -  Run {run}")
