"""
linear_modeling_exp3_s3.py - OLS, Ridge, ElNet on Experiment 3 Sub-3, no FS.

Feature set: SE2 (ADD+CONSIDER, 31 features) minus Inlet Total Coliform = 30 features.
Removing Coliform recovers ~347 rows lost in SE2, restoring train size to ~816 Grab / ~632 Comp.
No feature selection applied - full 30-feature set for all models.

Datasets: experiment3/sub_exp3/ (built by make_sub3_datasets.py)
Exp key:  Exp3-S3

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp3_s3/linear_modeling_exp3_s3.py
"""
...
DATASETS = [
    ("Exp3-S3", "E3S3_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp3-S3", "E3S3_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp3-S3", "E3S3_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp3-S3", "E3S3_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp3-S3", "E3S3_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp3-S3", "E3S3_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp3-S3", "E3S3_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp3-S3", "E3S3_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp3-S3 (ADD+CONSIDER minus Coliform, no FS)  |  Test R² Comparison (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp3-S3_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp3-S3 (ADD+CONSIDER minus Coliform, no FS)  -  Run {run}")
