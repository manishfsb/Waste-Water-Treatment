"""
linear_modeling_exp9_s3.py - OLS, Ridge, ElNet on Experiment 9 SE3 datasets.

Recency hypothesis - W1 + log1p target transformation.
Feature set: identical to Exp9-SE1 (27 features). Only BOD/COD/TSS targets are
log1p-transformed before fitting; pH is left on its natural scale.
Duan's smearing estimator is used to back-transform predictions to the original scale.

Why: BOD/COD/TSS are right-skewed non-negative concentrations. Linear models fit on
raw values are pulled by spikes; fitting on log1p makes the linear assumption more
valid and reduces heteroscedasticity. All reported metrics are on the ORIGINAL scale.

Datasets: experiment9/sub_exp1/ (same as SE1)
Exp key : Exp9-SE3

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp9_s3/linear_modeling_exp9_s3.py
"""
...
DATASETS = [
    ("Exp9-SE3", "Exp9W1Log_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)",       True),
    ("Exp9-SE3", "Exp9W1Log_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)",       True),
    ("Exp9-SE3", "Exp9W1Log_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)",       True),
    ("Exp9-SE3", "Exp9W1Log_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)",              False),
    ("Exp9-SE3", "Exp9W1Log_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)",  True),
    ("Exp9-SE3", "Exp9W1Log_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)",  True),
    ("Exp9-SE3", "Exp9W1Log_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)",  True),
    ("Exp9-SE3", "Exp9W1Log_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)",         False),
]
...
def train_dataset(experiment, ds_id, path, features, target, log_y, run):
    ...
    print(f"  Train: {len(train_df):>4d} rows | Test: {len(test_df):>3d} rows | "
          f"Features: {len(features)} | log_y={log_y}")

    # Transform targets
    if log_y:
        y_train_t = _log_transform(y_train)
    else:
        y_train_t = y_train
    ...
    preds[f"predicted_OLS_log_run_{run}"] = np.round(all_ols, 3)
    joblib.dump({"scaler": scaler, "model": ols, "log_y": log_y,
                 "smear": results.get("OLS_smear", 1.0)},
    ...
    preds[f"predicted_Ridge_log_run_{run}"] = np.round(all_ridge, 3)
    joblib.dump({"scaler": scaler, "model": ridge, "log_y": log_y,
                 "smear": results.get("Ridge_smear", 1.0)},
    ...
    preds[f"predicted_ElNet_log_run_{run}"] = np.round(all_elnet, 3)
    joblib.dump({"scaler": scaler, "model": elnet, "log_y": log_y,
                 "smear": results.get("ElNet_smear", 1.0)},
...
def _plot_scatter(ds_id, run, log_y, y_test, te_ols, te_ridge, te_elnet):
    ...
    tag = " (log1p target)" if log_y else ""
    fig.suptitle(f"{ds_id}  |  Test Set  -  W1+LogY{tag} (run {run})",
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp9-SE3 (W1+LogY)  |  Test R² (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
                df_results[["OLS_test_R2", "Ridge_test_R2", "ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp9SE3_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def save_results(all_results: list, run: int):
    ...
    key_cols = ["experiment", "dataset", "log_y", "OLS_test_R2", "Ridge_test_R2", "ElNet_test_R2"]
    print(df_new[key_cols].to_string(index=False))

# -- Main -----------------------------------------------------------------------
def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp9-SE3 (W1+LogY: 2024-only + log1p target transform)")
    ...
    print(f"\nExp9-SE3 Linear Modeling  -  Run {run} complete.")
