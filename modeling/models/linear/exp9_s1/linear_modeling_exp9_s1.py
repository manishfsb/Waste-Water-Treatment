"""
linear_modeling_exp9_s1.py - OLS, Ridge, ElasticNet on Experiment 9 SE1 datasets.

Recency hypothesis - W1 (2024-only training window).
Feature set: identical to Exp3-S1 (27 features: Inlet + Secondary + COMMON_CYCLIC
+ ADD-tier aeration). Only the training window changes: TRAIN_YEARS = [2024].

Datasets: experiment9/sub_exp1/
Exp key : Exp9-SE1

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp9_s1/linear_modeling_exp9_s1.py
"""
...
DATASETS = [
    ("Exp9-SE1", "Exp9W1_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp9-SE1", "Exp9W1_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp9-SE1", "Exp9W1_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp9-SE1", "Exp9W1_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp9-SE1", "Exp9W1_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp9-SE1", "Exp9W1_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp9-SE1", "Exp9W1_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp9-SE1", "Exp9W1_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp9-SE1 (W1: 2024-only)  |  Test R² Comparison (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15, df_results[["OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp9SE1_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp9-SE1 (W1: 2024-only training window)  -  Run {run}")
