"""
linear_modeling_exp3_s3_fs.py - OLS (LassoCV FS), Ridge, ElNet on Experiment 3 Sub-3.

Reads the SE3 datasets (ADD+CONSIDER minus Inlet Total Coliform, 30 features) from
experiment3/sub_exp3/ and applies model-specific feature selection:
...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp3_s3_fs/linear_modeling_exp3_s3_fs.py
"""
...
DATASETS = [
    ("Exp3-S3-FS", "E3S3_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp3-S3-FS", "E3S3_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp3-S3-FS", "E3S3_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp3-S3-FS", "E3S3_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp3-S3-FS", "E3S3_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp3-S3-FS", "E3S3_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp3-S3-FS", "E3S3_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp3-S3-FS", "E3S3_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp3-S3-FS (LassoCV FS on ADD+CONSIDER minus Coliform)  |  Test R² (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp3-S3-FS_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150); plt.close(fig)
...
def main():
    run = get_run_number()
    print(f"Linear Modeling  -  Exp3-S3 (LassoCV FS on ADD+CONSIDER minus Coliform)")
    ...
    print(f"\nExp3-S3 Linear FS  -  Run {run} complete")
