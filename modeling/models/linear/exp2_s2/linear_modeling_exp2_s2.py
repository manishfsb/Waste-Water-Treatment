...
Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s2/linear_modeling_exp2_s2.py
"""
...
DATASETS = [
    ("Exp2-Sub2", "E2S2_Grab_BOD", _cyc("grab_BOD"), E2S2_GRAB, "Effluent BOD (mg/L, Grab)"),
    ("Exp2-Sub2", "E2S2_Grab_COD", _cyc("grab_COD"), E2S2_GRAB, "Effluent COD (mg/L, Grab)"),
    ("Exp2-Sub2", "E2S2_Grab_TSS", _cyc("grab_TSS"), E2S2_GRAB, "Effluent TSS (mg/L, Grab)"),
    ("Exp2-Sub2", "E2S2_Grab_pH",  _cyc("grab_pH"),  E2S2_GRAB, "Effluent pH (Grab)"),
    ("Exp2-Sub2", "E2S2_Comp_BOD", _cyc("comp_BOD"), E2S2_COMP, "Effluent BOD (mg/L, Composite)"),
    ("Exp2-Sub2", "E2S2_Comp_COD", _cyc("comp_COD"), E2S2_COMP, "Effluent COD (mg/L, Composite)"),
    ("Exp2-Sub2", "E2S2_Comp_TSS", _cyc("comp_TSS"), E2S2_COMP, "Effluent TSS (mg/L, Composite)"),
    ("Exp2-Sub2", "E2S2_Comp_pH",  _cyc("comp_pH"),  E2S2_COMP, "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp2-Sub2  |  Test R² Comparison (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
        df_results[["OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-Sub2_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"Exp2-Sub2 Linear Modeling - Run {run}")
