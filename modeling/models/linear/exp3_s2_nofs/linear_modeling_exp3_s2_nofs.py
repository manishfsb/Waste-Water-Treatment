"""
linear_modeling_exp3_s2_nofs.py - OLS, Ridge, ElNet on Experiment 3 Sub-2, no FS.

Feature set: Exp2-Sub2 baseline (21) + ADD-tier (7 aeration) + CONSIDER-tier (4) = 32 features.
No feature selection applied - full 32-feature set for all models.

Datasets: experiment3/sub_exp2/ (built by make_sub2_datasets.py)
Exp key:  Exp3-S2

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp3_s2_nofs/linear_modeling_exp3_s2_nofs.py
"""
...
DATASETS = [
    ("Exp3-S2", "E3S2_Grab_BOD", _ds("grab_BOD"), "Effluent BOD (mg/L, Grab)"),
    ("Exp3-S2", "E3S2_Grab_COD", _ds("grab_COD"), "Effluent COD (mg/L, Grab)"),
    ("Exp3-S2", "E3S2_Grab_TSS", _ds("grab_TSS"), "Effluent TSS (mg/L, Grab)"),
    ("Exp3-S2", "E3S2_Grab_pH",  _ds("grab_pH"),  "Effluent pH (Grab)"),
    ("Exp3-S2", "E3S2_Comp_BOD", _ds("comp_BOD"), "Effluent BOD (mg/L, Composite)"),
    ("Exp3-S2", "E3S2_Comp_COD", _ds("comp_COD"), "Effluent COD (mg/L, Composite)"),
    ("Exp3-S2", "E3S2_Comp_TSS", _ds("comp_TSS"), "Effluent TSS (mg/L, Composite)"),
    ("Exp3-S2", "E3S2_Comp_pH",  _ds("comp_pH"),  "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    ax.set_title(f"Exp3-S2 (ADD+CONSIDER, no FS)  |  Test R² Comparison (run {run})",
                 fontsize=12)
    ax.legend(fontsize=9)
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp3-S2_r2_comparison_run_{run}.png")
...
def main():
    ...
    print(f"Linear Modeling  -  Exp3-S2 (ADD+CONSIDER, no FS)  -  Run {run}")
