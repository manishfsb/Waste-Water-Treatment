"""
linear_modeling_exp3_s1_fs.py - OLS, Ridge, and ElasticNet on Exp3 Sub-1 Feature-Selected subsets.

Identical training protocol to linear_modeling_exp3_s1.py.
Differences:
  - Reads from experiment3/sub_exp1/feature_selected_datasets/ (built by build_exp3_s1_fs_subsets.py)
  - Experiment label: "Exp3-S1-FS"
  - Writes all artifacts to models/linear/exp3_s1_fs/ (results.xlsx, models/, plots/)

Usage (from project root):
    .venv/bin/python3 21-25/modeling/models/linear/exp3_s1_fs/linear_modeling_exp3_s1_fs.py
"""
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    for exp in df_results["experiment"].unique():
        ...
        labels = [d.replace("Exp3S1FS_", "").replace("_", " ") for d in sub["dataset"]]
...
def main():
    ...
    print(f"Linear Modeling - Experiment 3 Sub-1 (Feature Selected) - Run {run}")
