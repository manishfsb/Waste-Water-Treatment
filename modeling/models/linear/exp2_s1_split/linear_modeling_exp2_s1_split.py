"""
linear_modeling_exp2_s1_split.py

Trains OLS, Ridge, and ElasticNet on Experiment 2 Sub-experiment 1 split variants:
...
No OLS_full_* columns are produced (there is no FS split).

Usage (from project root):
    .venv/bin/python3 modeling/models/linear/exp2_s1_split/linear_modeling_exp2_s1_split.py
"""
...
DATASETS = [
    # Clarifier
    ("Exp2-Sub1-Clr", "Clr_Grab_BOD", _clr("grab_BOD"), CLR_FEAT, "Effluent BOD (mg/L, Grab)"),
    ("Exp2-Sub1-Clr", "Clr_Grab_COD", _clr("grab_COD"), CLR_FEAT, "Effluent COD (mg/L, Grab)"),
    ("Exp2-Sub1-Clr", "Clr_Grab_TSS", _clr("grab_TSS"), CLR_FEAT, "Effluent TSS (mg/L, Grab)"),
    ("Exp2-Sub1-Clr", "Clr_Grab_pH",  _clr("grab_pH"),  CLR_FEAT, "Effluent pH (Grab)"),
    ("Exp2-Sub1-Clr", "Clr_Comp_BOD", _clr("comp_BOD"), CLR_FEAT, "Effluent BOD (mg/L, Composite)"),
    ("Exp2-Sub1-Clr", "Clr_Comp_COD", _clr("comp_COD"), CLR_FEAT, "Effluent COD (mg/L, Composite)"),
    ("Exp2-Sub1-Clr", "Clr_Comp_TSS", _clr("comp_TSS"), CLR_FEAT, "Effluent TSS (mg/L, Composite)"),
    ("Exp2-Sub1-Clr", "Clr_Comp_pH",  _clr("comp_pH"),  CLR_FEAT, "Effluent pH (Composite)"),
    # Sedimentation
    ("Exp2-Sub1-Sed", "Sed_Grab_BOD", _sed("grab_BOD"), SED_FEAT, "Effluent BOD (mg/L, Grab)"),
    ("Exp2-Sub1-Sed", "Sed_Grab_COD", _sed("grab_COD"), SED_FEAT, "Effluent COD (mg/L, Grab)"),
    ("Exp2-Sub1-Sed", "Sed_Grab_TSS", _sed("grab_TSS"), SED_FEAT, "Effluent TSS (mg/L, Grab)"),
    ("Exp2-Sub1-Sed", "Sed_Grab_pH",  _sed("grab_pH"),  SED_FEAT, "Effluent pH (Grab)"),
    ("Exp2-Sub1-Sed", "Sed_Comp_BOD", _sed("comp_BOD"), SED_FEAT, "Effluent BOD (mg/L, Composite)"),
    ("Exp2-Sub1-Sed", "Sed_Comp_COD", _sed("comp_COD"), SED_FEAT, "Effluent COD (mg/L, Composite)"),
    ("Exp2-Sub1-Sed", "Sed_Comp_TSS", _sed("comp_TSS"), SED_FEAT, "Effluent TSS (mg/L, Composite)"),
    ("Exp2-Sub1-Sed", "Sed_Comp_pH",  _sed("comp_pH"),  SED_FEAT, "Effluent pH (Composite)"),
]
...
def _plot_r2_barchart(df_results: pd.DataFrame, run: int):
    ...
    w = 0.25
    fig, ax = plt.subplots(figsize=(max(12, len(df_results) * 0.9), 5))
    ...
    ax.set_title(f"Exp2-Sub1-Split  |  Test R² Comparison (run {run})", fontsize=12)
    ax.legend(fontsize=9)
    ax.set_ylim(bottom=min(-0.15,
        df_results[["OLS_test_R2","Ridge_test_R2","ElNet_test_R2"]].min().min() - 0.05))
    plt.tight_layout()
    path = os.path.join(PLOTS_DIR, f"Exp2-Sub1-Split_r2_comparison_run_{run}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"Exp2-Sub1-Split Linear Modeling - Run {run}")
    print(f"Clarifier: {len(CLR_FEAT)} features | Sed: {len(SED_FEAT)} features\n")
