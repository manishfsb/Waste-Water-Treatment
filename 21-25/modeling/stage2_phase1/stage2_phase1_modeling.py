"""
stage2_phase1_modeling.py — Stage 2 Phase 1: Secondary clarifier features → Effluent targets

Replaces inlet features with secondary clarifier + secondary sedimentation measurements.
All 8 models share the same feature set — there is no grab/composite distinction
in the secondary clarifier data, only in the effluent targets.

Feature set
-----------
  Sec Clarifier : pH, TSS, BOD, COD, RAS
  Sec Sed       : pH, TSS, BOD, COD, RAS (New)
  Operational   : Flow (MLD), Power Total (KW)
...
Outputs
-------
  stage2_phase1/data/    — subset Excel files with predictions
  stage2_phase1/models/  — saved RF models (.pkl)
  stage2_phase1/plots/   — scatter, time-series, feature importance plots
  stage2_phase1/results.xlsx
"""
...
BASE_DIR     = os.path.dirname(SCRIPT_DIR)                       # modeling/
...
# ── Feature set (same for all 8 models) ───────────────────────────────────────
SEC_FEATURES = [
    "Sec Clarifier pH",
...
# (model_name, target_column, min_year)
MODELS = [
    ("stage2_p1_grab_BOD", "Effluent BOD (mg/L, Grab)",      2021),
    ("stage2_p1_grab_COD", "Effluent COD (mg/L, Grab)",      2021),
    ("stage2_p1_grab_TSS", "Effluent TSS (mg/L, Grab)",      2021),
    ("stage2_p1_grab_pH",  "Effluent pH (Grab)",             2021),
    ("stage2_p1_comp_BOD", "Effluent BOD (mg/L, Composite)", 2022),
    ("stage2_p1_comp_COD", "Effluent COD (mg/L, Composite)", 2022),
    ("stage2_p1_comp_TSS", "Effluent TSS (mg/L, Composite)", 2022),
    ("stage2_p1_comp_pH",  "Effluent pH (Composite)",        2022),
]
...
def prepare_subset(df: pd.DataFrame, target: str, min_year: int) -> pd.DataFrame:
    cols = ["Date", "year", "month", "day_of_week"] + SEC_FEATURES + [target]
    cols = list(dict.fromkeys(cols))
    sub  = df[df["year"] >= min_year][cols].copy()
    before = len(sub)
    sub  = sub.dropna(subset=SEC_FEATURES + [target])
...
def train_model(name: str, df_sub: pd.DataFrame, target: str):
    ...
    X_train, y_train = train[SEC_FEATURES].values, train[target].values
    X_test,  y_test  = test[SEC_FEATURES].values,  test[target].values
...
def save_subset(name: str, df_sub: pd.DataFrame, target: str, rf, run: int):
    path      = os.path.join(DATA_DIR, f"{name}.xlsx")
    rf_preds  = rf.predict(df_sub[SEC_FEATURES].values)

    if os.path.exists(path):
        df_existing = pd.read_excel(path)
        df_existing[f"predicted_RF_run_{run}"] = rf_preds.round(3)
        df_existing.to_excel(path, index=False)
    else:
        df_out = df_sub.copy()
        df_out[f"predicted_RF_run_{run}"] = rf_preds.round(3)
...
def plot_timeseries(name: str, df_sub: pd.DataFrame, target: str,
    ...
    df_plot["rf_pred"] = rf.predict(df_plot[SEC_FEATURES].values)
...
def plot_importance(name: str, rf, run: int):
    imps  = rf.feature_importances_
    order = np.argsort(imps)
    fig, ax = plt.subplots(figsize=(7, 6))
    ax.barh([SEC_FEATURES[i] for i in order], imps[order],
            color="#006B6B", edgecolor="white")
...
def main():
    ...
    for name, target, min_year in MODELS:
        print(f"\n{'─'*60}")
        print(f"Model  : {name}")
        ...
        df_sub = prepare_subset(df, target, min_year)
        result, rf, eval_data = train_model(name, df_sub, target)
        if result is None:
            continue
        ...
        save_subset(name, df_sub, target, rf, run)
        plot_scatter(name, test_df, target, rf_test_preds, run)
        plot_timeseries(name, df_sub, target, rf, run)
        plot_importance(name, rf, run)
