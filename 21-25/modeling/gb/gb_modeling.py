"""
gb_modeling.py — Gradient Boosting run across all stages and phases.

Trains GradientBoostingRegressor on the same data subsets used for RF,
using identical train/test splits. Results saved to gb/results.xlsx.
GB predictions appended as 'predicted_GB_run_N' to existing subset Excel files.
...
Usage (from project root):
    .venv/bin/python3 21-25/modeling/gb/gb_modeling.py
"""
...
TEST_YEAR   = 2025
GB_PARAMS   = dict(n_estimators=300, learning_rate=0.05,
...
# ── Model registry ─────────────────────────────────────────────────────────────
# (stage_label, model_name, subset_excel_path, features, target)
...
def get_gb_run_number(subset_path: str) -> int:
    """Check existing GB prediction columns to determine next run number."""
    if not os.path.exists(subset_path):
        return 1
    cols = pd.read_excel(subset_path, nrows=0).columns.tolist()
    return sum(1 for c in cols if c.startswith("predicted_GB_run_")) + 1


def append_prediction(subset_path: str, predictions: np.ndarray, run: int):
    """Append GB predictions as a new column to the existing subset Excel."""
    df = pd.read_excel(subset_path)
    df[f"predicted_GB_run_{run}"] = predictions.round(3)
...
def train_model(stage, name, subset_path, features, target):
    ...
    # Stage 1 uses 2020 for some models — keep consistent with RF
    ...
    results = {
        "stage":          stage,
        "model":          name,
        "GB_RMSE_train":  rmse(y_train, gb.predict(X_train)),
        "GB_R2_train":    r2_score(y_train, gb.predict(X_train)),
        "GB_RMSE_test":   rmse(y_test,  gb.predict(X_test)),
        "GB_R2_test":     r2_score(y_test,  gb.predict(X_test)),
    }
    return results, gb
...
def save_results(all_results: list, run: int):
    ...
    print(df_new[["stage", "model", "GB_RMSE_test", "GB_R2_test"]].to_string(index=False))


# ── Main ───────────────────────────────────────────────────────────────────────

def main():
    # Determine run number from first model's subset
    run = get_gb_run_number(MODELS[0][2])
    print(f"Starting GB run {run} across {len(MODELS)} models...\n")
    ...
    for stage, name, subset_path, features, target in MODELS:
        ...
        result, gb = train_model(stage, name, subset_path, features, target)
        if result is None:
            continue

        print(f"    GB  — RMSE: {result['GB_RMSE_test']:.3f}  "
              f"R²: {result['GB_R2_test']:.3f}")

        # Save model
        model_path = os.path.join(MODELS_DIR, f"{name}_GB_run_{run}.pkl")
        joblib.dump(gb, model_path)
        print(f"    Saved model → {model_path}")

        # Append predictions to existing subset file (all rows)
        df_all = pd.read_excel(subset_path)
        all_preds = gb.predict(df_all[features].values)
