"""
generate_report_elasticnet.py — RF vs ElasticNet comparison report across all stages.

Usage (from project root):
    .venv/bin/python3 21-25/modeling/elasticnet/generate_report_elasticnet.py
"""
...
ELNET_RESULTS = os.path.join(SCRIPT_DIR, "results.xlsx")
...
def load_elnet_results() -> pd.DataFrame:
    df = pd.read_excel(ELNET_RESULTS)
    return df[df["run"] == df["run"].max()].copy()


def get_elnet_run() -> int:
    return int(pd.read_excel(ELNET_RESULTS)["run"].max())


# ── Chart builders ─────────────────────────────────────────────────────────────

def r2_heatmap(df_rf: pd.DataFrame, df_elnet: pd.DataFrame) -> go.Figure:
    model_names = [f"{st}_{mt}" for mt in TARGETS for st in ["grab", "comp"]]
    col_labels  = []
    r2_matrix   = []

    for stage in STAGES:
        for algo, df in [("RF", df_rf), ("ElNet", df_elnet)]:
            ...
            for mshort in model_names:
                if algo == "RF":
                    ...
                    match = df[(df["stage"] == stage) & (df["model"].str.endswith(mshort))]
                    val = float(match["ElNet_R2_test"].values[0]) if len(match) else None
    ...
    fig.update_layout(
        title="R² Test Set — All Models, All Stages, RF vs ElasticNet",
...
def delta_chart(df_rf: pd.DataFrame, df_elnet: pd.DataFrame) -> go.Figure:
    ...
    for stage in STAGES:
        for st in ["grab", "comp"]:
            for t in TARGETS:
                ...
                elnet_row = df_elnet[(df_elnet["stage"] == stage) & (df_elnet["model"].str.endswith(suffix))]
                if len(rf_row) and len(elnet_row):
                    d = (float(elnet_row["ElNet_R2_test"].values[0]) -
    ...
    fig = go.Figure(go.Bar(
        ...
        hovertemplate="%{y}<br>ΔR² (ElNet−RF): %{x:+.3f}<extra></extra>",
    ))
    fig.add_vline(x=0, line_dash="dash", line_color="black", line_width=1)
    fig.update_layout(
        title="ΔR² = ElasticNet minus RF  (positive = ElasticNet better)",
...
def summary_table_html(df_rf: pd.DataFrame, df_elnet: pd.DataFrame) -> str:
    ...
    def winner(rf_r2, elnet_r2):
        if rf_r2 is None or elnet_r2 is None:
            return '<td style="text-align:center">—</td>'
        if elnet_r2 > rf_r2 + 0.01:
            return '<td style="text-align:center;color:#238B45;font-weight:bold">ElNet ▲</td>'
        elif rf_r2 > elnet_r2 + 0.01:
    ...
    for stage in STAGES:
        ...
        for st in ["grab", "comp"]:
            for t in TARGETS:
                ...
                elnet_rmse_tr = float(elnet_row["ElNet_RMSE_train"].values[0]) if len(elnet_row) else None
                elnet_r2_tr   = float(elnet_row["ElNet_R2_train"].values[0])   if len(elnet_row) else None
                elnet_rmse = float(elnet_row["ElNet_RMSE_test"].values[0])  if len(elnet_row) else None
                elnet_r2   = float(elnet_row["ElNet_R2_test"].values[0])    if len(elnet_row) else None
                ...
                <tr>
                  ...
                  {cell(elnet_rmse_tr)}{cell(elnet_r2_tr, True)}
                  {cell(elnet_rmse)}{cell(elnet_r2, True)}
                  {winner(rf_r2, elnet_r2)}
                </tr>"""
    ...
    <table class="summary-table">
      <thead>
        <tr>
          ...
          <th colspan="2" style="background:#08519C">ElNet (train)</th>
          <th colspan="2" style="background:#2171B5">ElNet (test)</th>
...
def observations_html(df_rf: pd.DataFrame, df_elnet: pd.DataFrame) -> str:
    elnet_wins, rf_wins, ties = 0, 0, 0
    for stage in STAGES:
        for st in ["grab", "comp"]:
            for t in TARGETS:
                suffix = f"{st}_{t}"
                rf_row = df_rf[(df_rf["stage"] == stage) & (df_rf["model"].str.endswith(suffix))]
                elnet_row = df_elnet[(df_elnet["stage"] == stage) & (df_elnet["model"].str.endswith(suffix))]
                if not len(rf_row) or not len(elnet_row):
                    continue
                rf_r2 = float(rf_row["RF_R2_test"].values[0])
                elnet_r2 = float(elnet_row["ElNet_R2_test"].values[0])
                d = elnet_r2 - rf_r2
                if d > 0.01:   elnet_wins += 1
                elif d < -0.01: rf_wins += 1
                else:          ties += 1

    best_elnet = df_elnet.loc[df_elnet["ElNet_R2_test"].idxmax()]
    best_rf = df_rf.loc[df_rf["RF_R2_test"].idxmax()]

    return f"""
    <ul>
      <li>ElasticNet outperforms RF in <b>{elnet_wins}</b> models,
          RF outperforms ElasticNet in <b>{rf_wins}</b>,
          approximately tied in <b>{ties}</b>.</li>
      <li>Best overall ElasticNet model: <b>{best_elnet['model']}</b>
          [{best_elnet['stage']}] — R² = {best_elnet['ElNet_R2_test']:.3f}</li>
...
<head>
  <meta charset="UTF-8">
  <title>RF vs ElasticNet Comparison Report — Run {run}</title>
  {plotly_cdn}
  <style>{CSS}</style>
</head>
<body>
<div class="container">
  <h1>RF vs ElasticNet Regression — Comparison Report</h1>
  <p class="meta">
    All stages (Stage 1, Stage 2 P1, Stage 2 P2) &nbsp;|&nbsp;
    Test year: 2025 &nbsp;|&nbsp; ElasticNet run: <b>{run}</b>
  </p>

  <h2>Overview Heatmap</h2>
  <div class="card">{chart_div(r2_heatmap(df_rf, df_elnet))}</div>

  <h2>ΔR² — Where ElasticNet beats RF (and where it doesn't)</h2>
  <div class="card">{chart_div(delta_chart(df_rf, df_elnet))}</div>

  <h2>Full Summary Table</h2>
  <div class="card">{summary_table_html(df_rf, df_elnet)}</div>

  <h2>Key Observations</h2>
  <div class="obs-card">{observations_html(df_rf, df_elnet)}</div>
</div>
</body>
</html>"""
...
def main():
    run = get_elnet_run()
    print(f"Generating RF vs ElasticNet report for ElasticNet run {run}...")
    html = build_report(run)
    out_path = os.path.join(SCRIPT_DIR, f"report_elasticnet_run_{run}.html")
