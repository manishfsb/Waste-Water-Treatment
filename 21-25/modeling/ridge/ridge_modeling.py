"""
ridge_modeling.py — Ridge regression across all stages and phases.

Reads existing subset Excel files produced by the stage modeling scripts.
Tunes alpha via GridSearchCV + TimeSeriesSplit. Results saved to
ridge/results.xlsx. Predictions appended as 'predicted_Ridge_run_N'.
...
Usage (from project root):
    .venv/bin/python3 21-25/modeling/ridge/ridge_modeling.py
"""
...
def get_run_number(subset_path: str) -> int:
    ...
    return sum(1 for c in cols if c.startswith("predicted_Ridge_run_")) + 1


def append_prediction(subset_path: str, predictions: np.ndarray, run: int):
    df = pd.read_excel(subset_path)
    df[f"predicted_Ridge_run_{run}"] = predictions.round(3)
...
def train_model(stage, name, subset_path, features, target):
    ...
    scaler, ridge, cv_rmse, best_alpha = tune_ridge(X_train, y_train)
    train_pred = ridge.predict(scaler.transform(X_train))
    test_pred  = ridge.predict(scaler.transform(X_test))

    results = {
        "stage":             stage,
        "model":             name,
        "Ridge_RMSE_train":  rmse(y_train, train_pred),
        "Ridge_MAE_train":   mae(y_train,  train_pred),
        "Ridge_R2_train":    r2_score(y_train, train_pred),
        "Ridge_CV_RMSE":     cv_rmse,
        "Ridge_RMSE_test":   rmse(y_test,  test_pred),
        "Ridge_MAE_test":    mae(y_test,   test_pred),
        "Ridge_MAPE_test":   mape(y_test,  test_pred),
        "Ridge_R2_test":     r2_score(y_test, test_pred),
        "Ridge_alpha":       best_alpha,
    }

    # Predict on all rows for appending to subset file
    all_preds = ridge.predict(scaler.transform(df[features].values))
    return results, (scaler, ridge), all_preds
...
def save_results(all_results: list, run: int):
    ...
    print(df_new[["stage", "model", "Ridge_RMSE_test", "Ridge_R2_test"]].to_string(index=False))
...
def main():
    ...
    print(f"Starting Ridge run {run} across {len(MODELS)} models...\n")
    ...
    for stage, name, subset_path, features, target in MODELS:
        ...
        scaler, ridge = model_pair
        print(f"    Ridge — RMSE: {result['Ridge_RMSE_test']:.3f}  "
              f"R²: {result['Ridge_R2_test']:.3f}")

        model_path = os.path.join(MODELS_DIR, f"{name}_Ridge_run_{run}.pkl")
        joblib.dump({"scaler": scaler, "model": ridge}, model_path)
